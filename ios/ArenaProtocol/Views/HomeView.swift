...
struct HomeView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                // Fixed header — always visible, not scrollable
                fixedHeader

                // Tab strip — only when session is running
                if hasSession {
                    sessionTabBar
                }

                // Scrollable content — switches between home and session tab
        ...
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 4) {
                Text("ARENA PROTOCOL")
                    ...
                    .foregroundStyle(Color.white.opacity(hasSession ? 0.15 : 0.25))
                    .kerning(6)
        ...
        return VStack(alignment: .leading, spacing: 0) {
            ...
            if let next = nextSlot {
                ...
                HStack(spacing: 10) {
                    ...
                    Spacer()
                    // Clock time of next arena start in user's timezone
                    Text(formattedStartTime(next.start))
                        .font(.system(size: 13, weight: .medium, design: .monospaced))
                        .foregroundStyle(nc.opacity(0.65))
                }
                .padding(.horizontal, 20)
                .padding(.vertical, 10)
                if moreCount > 0 {
    ...
    /// Format a Date as a short clock time in the user's configured timezone.
